...
def plugin():
    ...
    p._rom_removal_service = RomRemovalService(
        config=RomRemovalServiceConfig(
            ...
            frontend=_frontend(roms=os.path.join(os.path.expanduser("~"), "retrodeck", "roms")),
...
class TestStartDownload:
    ...
    async def test_starts_download_task(self, plugin, tmp_path):
        ...
        plugin._download_service._frontend = _frontend(
            roms=str(tmp_path / "retrodeck" / "roms"), bios=str(tmp_path / "retrodeck" / "bios")
        )
        plugin._rom_removal_service._frontend = _frontend(roms=str(tmp_path / "retrodeck" / "roms"))
...
class TestCleanupLeftoverTmpFilesNoRetrodeckPaths:
    ...
    def test_empty_roms_and_bios_paths_skip_walk(self, plugin):
        ...
        fake = FakeDownloadFileStore()
        plugin._download_service._download_file_store = fake
        # frontend present but both helpers return empty (no
        # retrodeck.json) — service must early-return on each branch.
        plugin._download_service._frontend = _frontend(roms="", bios="")
